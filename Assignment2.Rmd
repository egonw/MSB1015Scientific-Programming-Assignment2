...
Subsqequently, the assignment is to build a model that can predict the boilingpoint of a chemical compound based on its chemical composition. In other words the assignment is derive physical properties from a compounds chemical structure. Therefore, chemical features such as wiener paramters [1] and atom count are used to build a regression model. 
...
print(cat("Mean boilingpoint:", mean(r$bp)))
print(cat("Smallest boilingpoint:", min(r$bp)))
print(cat("Highest boilingpoint:", max(r$bp)))
```
## Descriptors
...
# Selection of suitable selectors is crucial to be able to predict the boiling point of chemical compound with reasonable accuracy. The function get.desc.names() will print a list of descriptors available within the rcdk package. The user of this code may chose any set of descriptors that is considered to contain information relevant to predicting the boiling point. Adding it to the list of descriptor names will allow for computation of descriptors from the smiles. 
descNames <- c(
...
# Randomization has great influence on how the model is built (trainingset) and how it performs on the testset. Because the dataset is fairly small the way in which randomization occurs (set.seed()) is esential fro reproducibility of results. It is important to keep in mind that a different seed might produce different results. 
...
print(cat("Number of samples in trainingset: ", nrow(train)))
print(cat("Number of samples in testset: ", nrow(test)))
...
# plot error
ncompo <- c(1:ncol(data))
plot(ncompo, error$val, type = "l", pch = 16,  xlab = "Number of principle components",
     ylab = "RMSEP [K]", main = "RMSE of Prediction against Number of Principle Components")
# correlation of results 
corr <- c()
for (i in 1:ncol(data)) {
  corr[i] <- cor(predicted[,,i-1], ytest)
}
plot(ncompo[1:ncol(data)], corr, type = "p", xlab = "Number of principle components", ylab = "Correlation",
...
rf <- randomForest(train$r.bp~., data = train)
rf.pred <- predict(rf, test)
error.rf <- rmse(rf.pred, ytest)
```
